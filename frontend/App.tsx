...
//     // }
//     console.log(`http://${IP_ADDR}:${PORT}/register`);
//     try {
//       const response = await axios.post(`http://${IP_ADDR}:${PORT}/register`, formData, {
//         headers: {
...
import 'react-phone-input-2/lib/style.css';
import axios from 'axios';
import { IP_ADDR, PORT } from '@/config/parameters';
...
const MarathonRegistration: React.FC = () => {
  const [formData, setFormData] = useState<RegistrationFormData>({
    name: '',
    email: '',
    gender: '',
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8">
        <div className="text-center mb-8">
          <Runner className="mx-auto h-12 w-12 text-blue-600" />
          <h2 className="mt-6 text-3xl font-bold text-gray-900">Marathon Registration</h2>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
